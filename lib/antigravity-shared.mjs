...
export const CLIENT_ID = process.env.GMAIL_OAUTH_CLIENT_ID || '';
export const CLIENT_SECRET = process.env.GMAIL_OAUTH_CLIENT_SECRET || '';
export const SCOPES = [
...
export const DEFAULT_PASSWORD = process.env.GMAIL_PASSWORD || '';
...
export async function saveShot(page, email, label, opts = {}) {
  ...
  await page.screenshot({ path: file, fullPage: false, timeout: 10_000 }).catch((err) => {
    console.error(`  ⚠️  Screenshot failed (${label}): ${err.message}`);
  });
  return file;
}
